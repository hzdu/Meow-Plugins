...
const btnDomainQuery = document.getElementById('btn-domain-query');
...
const scratchModalSelectAllBtn = document.getElementById('scratch-modal-selectall');
const scratchModalCopyBtn = document.getElementById('scratch-modal-copy');
...
// ================== 数据加载与保存 ==================
// 带重试的 storage 读取：长时间未打开侧边栏时，Service Worker 可能已休眠，
// 首次唤醒 chrome.storage 偶发超时/连接断开，重试可大幅提升成功率。
async function _safeStorageGet(area, keys, retries = 2) {
    for (let attempt = 0; ; attempt++) {
        try {
            return await chrome.storage[area].get(keys);
        } catch (err) {
            console.warn(`Meow: storage.${area}.get failed (attempt ${attempt + 1}/${retries + 1}):`, err);
...
async function loadData() {
    ...
    let localData = {};
    try {
        syncData = await _safeStorageGet('sync', ['meow_prompts', 'prompt_categories', 'meow_scratchpad_list', 'meow_read_later', 'meow_ai_tags', 'meow_hot_tab_order']);
    } catch (e) {
        console.error("Meow: loadData() sync storage 读取最终失败，使用空数据继续。", e);
    }
    try {
        localData = await _safeStorageGet('local', ['meow_gallery', 'meow_scratchpad', 'meow_scratchpad_list_local', 'meow_2fa_accounts', 'meow_2fa_auto_fetch', 'meow_prompts']);
    } catch (e) {
        console.error("Meow: loadData() local storage 读取最终失败，使用空数据继续。", e);
    }
...
async function initSidepanel() {
    // === 0. 初始化国际化 ===
    // 国际化失败不应阻塞后续渲染，仍可使用默认中文文案
    try {
        if (window.meowI18n) {
            await window.meowI18n.init();
        }
    } catch (err) {
        console.error("Meow: i18n init failed, continue with defaults.", err);
    }
    ...
    // === 初始化加载 ===
    // 长时间未打开侧边栏时，Service Worker 可能已休眠，
    // chrome.storage 首次唤醒偶发超时/连接断开，loadData 内部已加重试与容错。
    // 即便 loadData 最终仍失败，也要保证 UI 基础逻辑（Tab 恢复等）不被跳过。
    try {
        await loadData();
    } catch (err) {
        console.error("Meow: loadData() failed during initSidepanel, UI will render with empty data.", err);
    }

    // 将后续渲染拆分为独立 try/catch 块，单点失败不阻塞后续
    try {
        renderCategories();
        renderFilters();
        renderPromptList();
        ...
        console.error("Meow: render functions failed.", err);
    }
    ...
    try {
        ...
        console.error("Meow: paste handler setup failed.", err);
    }
    ...
    try {
        ...
        console.error("Meow: applySavedHotTabOrder failed.", err);
    }
    ...
    try {
        ...
        console.error("Meow: fetchHotList failed.", err);
    }
    ...
    try {
        ...
        console.error("Meow: restoreSpTabOrder failed.", err);
    }
    ...
    try {
        ...
        console.error("Meow: initSpTabDragSort failed.", err);
    }

    // === 恢复上次停留的标签页 ===
    // 此步骤非常关键：它负责切换显示对应的 view-content。
    // 即使前面的渲染都失败了，这一步也要执行，否则所有 view 都是 hidden。
    try {
        restoreLastTab();
    } catch (err) {
        console.error("Meow: restoreLastTab failed, fallback to default active tab.", err);
        // 兜底：直接点击默认激活的 tab 按钮，确保至少有一个视图显示
        try {
            const defaultBtn = document.querySelector('.tab-btn.active') || document.querySelector('.tab-btn');
            if (defaultBtn) defaultBtn.click();
        } catch (e) {
            console.error("Meow: fallback tab click also failed.", e);
        }
    }

    console.log("Meow Sidepanel: initSidepanel finished.");
}
...
document.addEventListener('keydown', function(e) {
    ...
    if ((e.ctrlKey || e.metaKey) && (e.key === 's' || e.key === 'S')) {
        if (promptViewModal && !promptViewModal.classList.contains('hidden')) {
            e.preventDefault();
            promptViewSaveBtn.click();
        } else if (scratchEditModal && !scratchEditModal.classList.contains('hidden')) {
            e.preventDefault();
            scratchModalSaveBtn.click();
        }
    }
    // Alt+F1 切换标签页
