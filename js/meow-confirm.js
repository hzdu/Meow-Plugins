...
(function () {
    ...
    function showPromptDialog(options) {
        ...
        return new Promise(function (resolve) {
            ...
            overlay.innerHTML =
                '<div class="meow-confirm-dialog" role="dialog" aria-modal="true">' +
                '   <div class="meow-confirm-body">' +
                '       <div class="meow-confirm-title">' + _escapeHtml(title) + '</div>' +
                '       <div class="meow-prompt-input-wrap">' +
                '           <input type="text" class="meow-prompt-input" placeholder="' + _escapeHtml(placeholder) + '" value="' + _escapeHtml(defaultValue) + '">' +
                '       </div>' +
                '   </div>' +
                '   <div class="meow-confirm-actions">' +
                '       <button class="meow-confirm-btn meow-confirm-btn-cancel" data-result="false">' + _escapeHtml(cancelText) + '</button>' +
                '       <button class="meow-confirm-btn meow-confirm-btn-confirm info" data-result="true">' + _escapeHtml(confirmText) + '</button>' +
                '   </div>' +
                '</div>';
            ...
            var input = overlay.querySelector('.meow-prompt-input');
            var btns = Array.prototype.slice.call(overlay.querySelectorAll('.meow-confirm-btn'));
            var selected = 1;
            ...
            function finish(result) {
                ...
                document.removeEventListener('keydown', onKeydown);
                setTimeout(function () { overlay.remove(); }, 250);
                resolve(result ? input.value.trim() || null : null);
            }
